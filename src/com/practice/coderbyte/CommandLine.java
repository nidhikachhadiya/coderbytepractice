...
public class CommandLine {
    public static String CommandLine(String str) {
      boolean isLastCharEqual=false;
        if(str.charAt(str.length()-1)=='='){
            isLastCharEqual = true;
        }
        String Stringnew="";
        String strArray[]=str.split("=");
        Stringnew = Stringnew + strArray[0].length();
        if(strArray.length>1) {
            for (int i = 1; i < strArray.length; i++) {
                for (int j = strArray[i].length() - 1; j >= 0; j--) {
                    if (Character.isSpaceChar(strArray[i].charAt(j))) {
                        int count = strArray[i].length() - 1 - j;
                        Stringnew = Stringnew + "=" + j + " " + count;
                        break;
                    }
                }
              ...
              if(!isLastCharEqual) {
                  return Stringnew + "=" + strArray[strArray.length - 1].length();
              }
              else
                  return Stringnew + "=" + 0;
    ...
    public static void main (String[] args) {
        ...
        // String s="a=3 b=4 a=23 b=a 4 23 c=";
        String s="origin=2;3 destination=7;9 stops= 3;6 8;9 12;17";
        System.out.print(CommandLine(s));
    }
}
